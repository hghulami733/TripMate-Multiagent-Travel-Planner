...
# Default origin when only a destination is mentioned, e.g. "plan a Japan trip".
# Must be a real 3-letter IATA code (e.g. "LHR"), or leave unset. "EUROPE" is
# NOT a valid IATA code and was silently sent straight to the AviationStack
# API before this fix -- if the code isn't a real airport we know about, we
# fall back to no default origin rather than sending garbage to the API.
_raw_default_origin = os.getenv("DEFAULT_ORIGIN_IATA", "").strip().upper()
DEFAULT_ORIGIN_IATA = _raw_default_origin if _raw_default_origin in AIRPORTS else None
...
def parse_route(query: str):
    ...
    match = re.search(r"\bfrom\s+(.+?)(?:\s+(?:on|for|under|including|with|in|at)\b|[.!?]|$)", q_lower)
    if match:
        origin_text = match.group(1)
        dep_iata = resolve_location_to_iata(origin_text)
    ...
    match = re.search(r"\bto\s+(.+?)(?:\s+(?:on|for|under|including|with|in|at)\b|[.!?]|$)", q_lower)
